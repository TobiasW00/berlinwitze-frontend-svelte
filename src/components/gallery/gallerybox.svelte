...
$: member = (logedin) ?  $Memberstore.find(f=>f.id ===gallery.ownerid):false;
    ...
    if($user !==undefined)
    {
        addAction("user.single", gallery.ownerid, function(data){
            Memberstore.updateMember(data.content);	
        });
    }
...
<a class="boxpicture" href="/gallery/{gallery.id}/{gallery.title}">
    ...
    <img on:error={loaddefault} src="{HTTP_URL}/berlinwitzepictures/{gallery.id}/orginal_{gallery.pictures[0].id}.webp" alt={gallery.title} />
    {:else}
